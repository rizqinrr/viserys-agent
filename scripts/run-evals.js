#!/usr/bin/env node
/**
 * run-evals.js — skill eval runner for viserys.
 *
...
function materializeWorkspace(ev) {
  ...
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'viserys-eval-'));
...
function runBehavioral(skillName, dryRun) {
  ...
  for (const ev of d.evals) {
    ...
    const workspace = kind === 'dialogue'
      ? fs.mkdtempSync(path.join(os.tmpdir(), 'viserys-dialogue-eval-'))
