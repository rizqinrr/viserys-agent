...
- **Invoke via:** `/ship` (parallel fan-out alongside `maester`, `prover`, and `chronicler`), or any future `/audit` command.
- **Do not invoke from another persona.** If `maester` flags something that warrants a deeper security pass, the user or a slash command initiates that pass — not the reviewer. See [personas and orchestration](../docs/personas-and-orchestration.md).
